...
def classify_correlation(r):
    """
    Categorizes the absolute Spearman correlation to avoid strictly orthogonal terminology.
    """
    abs_r = abs(r)
    if abs_r < 0.2:
...
def run_pair_search(df_expr, df_meta, df_expr_val, df_meta_val, top_genes, df_thresh, alpha=0.2, beta=0.1, out_path=None):
    """
    Evaluates all pairwise combinations of top_genes.
    Applies thresholds and computes sensitivity, specificity, Spearman r, instability, and composite Pair Score.
    """
    # Mapping of gene to threshold and standard deviation (instability)
    ...
    for i in range(len(top_genes)):
        for j in range(i + 1, len(top_genes)):
            ...
            # Check availability in validation expression matrices
            if gene_a not in df_expr_val.index or gene_b not in df_expr_val.index:
                continue
                
            # Get expressions and min-max scale [0, 1] relative to discovery range
            # Gene A
            ...
            # Retrieve thresholds and instability
            K_a = gene_to_thresh[gene_a]
            K_b = gene_to_thresh[gene_b]
            
            instab_a = gene_to_instab[gene_a]
            instab_b = gene_to_instab[gene_b]
            
            # AND-gate activation (Boolean Logic)
            ...
            # Spearman correlation in tumor tissue
            ...
            pair_score = performance_score - redundancy_penalty - instability_penalty
    ...
    if out_path:
        ...
        print(f"[+] Saved pair search results to {out_path} (top pair: {df_pairs.iloc[0]['gene_A']} + {df_pairs.iloc[0]['gene_B']} with score {df_pairs.iloc[0]['pair_score']:.4f})")
