...
\input{../shared/report_style_revised.tex}
...
\author[1]{Chen-Jung Shih, Hsuan-Yu Liao, De-Fang Su, Chia-Yi Lin}
\affil[1]{Department of Synthetic Biology and Bioinformatics
...
\section{Limitations}
\begin{enumerate}
\item This is an in silico proof-of-concept; biochemical kinetics may differ.
\item SHAP-inferred thresholds are statistical inflection points and do not map directly to biochemical dissociation constants.
\item Bulk RNA-seq data reflects average cell populations and is highly influenced by stromal density and immune infiltration.
\item The comparison between TCGA (tumor) and GTEx (normal) may contain subtle batch effects despite TOIL harmonization.
\item External validation cohort demonstrated extremely low sensitivity (4.3%), indicating significant challenge in threshold transfer.
\item The selected candidate pair (UBE2S + CCR6) is not strictly statistically orthogonal, exhibiting a correlation of 0.714.
\item Transcriptomic abundance differences do not guarantee equivalent sensor accessibility or protein translation.
\item The final candidates require promoter engineering or RNA sensor design, which introduces additional complexity.
\item Any diagnostic or therapeutic application requires extensive wet-lab validation and safety testing in model organisms.
\end{enumerate}

\section{Proposed Wet-Lab Validation}
To validate this AND-gate biosensor in vitro, we propose a synthetic notch (synNotch) or synthetic promoter construct. The promoter regions driving UBE2S and CCR6 will be cloned upstream of two orthogonal transcription factors (e.g., tTA and LhG4). These factors will cooperatively drive a split-transactivator system, activating a green fluorescent protein (GFP) reporter or therapeutic payload (e.g., Diphtheria toxin A) only when both promoters are active. Testing should be performed on PDAC cell lines (e.g., PANC-1, MIA PaCa-2) as positive controls and normal human pancreatic duct epithelial cells (HPDE) as negative controls, followed by in vivo validation in tumor xenograft mouse models.
